...
export class DocumentConverter {
  ...
  async convertDocument(request: ConversionRequest): Promise<ConversionResult> {
    ...
    try {
      ...
      let intermediateContent: string;
      let intermediateFormat: string;
  ...
  private async convertDocxToHtml(base64Content: string): Promise<string> {
    try {
  ...
  private async convertPdfToText(base64Content: string): Promise<string> {
    try {
      const buffer = Buffer.from(base64Content, 'base64');
  ...
  private async convertHtmlToText(htmlContent: string): Promise<string> {
    try {
  ...
  private async convertHtmlToMarkdown(htmlContent: string): Promise<string> {
    try {
  ...
  private convertTextToMarkdown(textContent: string): string {
